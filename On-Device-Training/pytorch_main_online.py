...
def load_data():
    ...
    X_train = np.empty((0, n_ch, int(160*n_T/n_ds)))
    y_train = np.empty(0)
    for subject in subjects_training:
        ...
        X = eeg_reduction(X, n_ds=n_ds, n_ch=n_ch, T=n_T)  # Implement this function
    ...
    X_validation = np.empty((0, n_ch, int(160*n_T/n_ds)))
    ...
    X_testing = np.empty((0, n_ch, int(160*n_T/n_ds)))
...
def save_online_training_testing_data():
    ...
    for subject in subjects:
        ...
        X_train_online = np.reshape(X_train_online, (X_train_online.shape[0], 1, n_ch, int(160 * n_T / n_ds)))

        X_test_online = np.expand_dims(X_test_online, axis=-1)
        X_test_online = np.reshape(X_test_online, (X_test_online.shape[0], 1, n_ch, int(160 * n_T / n_ds)))
...
def training_offline(leave_one_session_out = False):
    ...
    X_train = np.reshape(X_train, (X_train.shape[0], 1, n_ch, int(160 * n_T / n_ds)))
    ...
    X_test = np.reshape(X_test, (X_test.shape[0], 1, n_ch, int(160 * n_T / n_ds)))
    ...
    torchinfo.summary(backbone, (16, 1, n_ch, int(160 * n_T / n_ds)), col_names=("input_size", "output_size", "num_params", "kernel_size", "mult_adds"), verbose=1)
    torchinfo.summary(classifier, (16, 16 * int(signal_length / 16)), col_names=("input_size", "output_size", "num_params", "kernel_size", "mult_adds"), verbose=1)
    ...
    for subject in subjects:
        ...
        with torch.no_grad():
            for batch_X, batch_y in test_loader:
                ...
                #print("y_pred_clas: ", y_pred_cls)
                #print("y_batch_clas: ", torch.argmax(batch_y, dim=1).view_as(y_pred_cls))
        accuracy = correct / len(test_loader.dataset)
...
def testing_offline_test_subject_1():
    #subjects_testing = range(1, 10)
    #testing_subjects = testing_subjects
    subjects = [x for x in testing_subjects if (x not in excluded_subjects)]
    ...
    for subject in subjects:
        ...
        #backbone.to(device)
        #classifier.to(device)
        #EEGNet = model_sep.EEGnet()
        #EEGNet.load_state_dict(torch.load(os.path.join(results_dir, f'model/offline_training_combined.onnx')))

        EEGNet = onnx.load(os.path.join(results_dir, f'model/offline_training_combined.onnx'))
        onnx.checker.check_model(EEGNet)

        ort_session = onnxruntime.InferenceSession(os.path.join(results_dir, f'model/offline_training_combined.onnx'), providers=["CPUExecutionProvider"])
        ...
        with torch.no_grad():
            for batch_X, batch_y in test_loader:
                ...
                ort_inputs = {ort_session.get_inputs()[0].name: to_numpy(batch_X)}
                ort_outs = ort_session.run(None, ort_inputs)
                #print(ort_outs[0])

                #y_pred = EEGNet(batch_X)
                y_pred_cls = np.argmax(ort_outs[0], axis=1)
                ##pred = y_pred_cls.argmax(dim=1, keepdim=True)
                #correct += y_pred_cls.eq(torch.argmax(batch_y, dim=1).view_as(y_pred_cls)).sum().item()
                if y_pred_cls == torch.argmax(batch_y, dim=1).cpu().detach().numpy():
                    correct += 1
                #print("y_pred_clas: ", y_pred_cls)
                #print("y_batch_clas: ", torch.argmax(batch_y, dim=1).cpu().detach().numpy())
        accuracy = correct / len(test_loader.dataset)
        Testing_offline_accuracy.append(accuracy)
        print("Subject: {}, testing accuracy: {}".format(subject, accuracy))
    print("All Testing Subject, averaged offline testing accuracy: {}".format(np.mean(Testing_offline_accuracy)))
...
def combine_backbone_classifier():
    ...
    dummy_input_backbone = torch.randn(1, 1, n_ch, int(160 * n_T / n_ds)).to(device)
...
if __name__ == '__main__':
    ...
    ## compress 1
    #n_ds = 1   # downsampling
    #n_ch = 19   # channel reduction
    #n_T = 2   # time window length
    #signal_length = 320
    ## compress 2
    #n_ds = 1  # downsampling
    #n_ch = 8  # channel reduction
    #n_T = 1  # time window length
    #signal_length = 160
    ...
    #datapath_save_subject_data = "/datasets/sibian/eeg/subjects_compress_1/"
    #datapath_save_subject_data = "/datasets/sibian/eeg/subjects_compress_2/"

    results_dir=f'results/{experiment_name}/'
    #results_dir=f'results_compress_1/{experiment_name}/'
    #results_dir = f'results_compress_2/{experiment_name}/'
    ...
    testing_offline_test_subject_1()